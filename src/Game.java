...
public class Game {
    ...
    // maps that reference a coordinate name to a ship or a ship to a list of the coordinates it occupies 
    // (e.g E4 -> Destroyer)
    ...
    public static void playerMoves(ArrayList<Ship> shipsAlive, ArrayList<String> playerShipsAlive) {
        ...
        // check if the players hit hits a ship and mark it on the ai placement board, cur is a placeholder variable name
        ...
        if (cur.getIsShip() && !cur.getIsHit()) {
            ...
            if (shipHit.getTimesHit() == shipHit.getSize()) {
                // must have a default constructor so we can remove the name of the ship from the list of ships alive
